...
只要任意一处不一致，客户端就无法正常连接。

如果你在 3X-UI 中设置的真实端口是：
...
如果 3X-UI 已经安装并且可以正常使用，不需要重新执行 3X-UI 安装脚本。
...
如果某个 `modprobe` 提示模块不存在，但后续 `nft` 命令可以正常执行，一般不影响使用。

## 4. 删除旧的端口转发规则

如果之前测试过其他端口，建议先删除旧的 `hy2jump` 规则表：
...
将 UDP `49000-50000` 重定向到 HY2 真实端口 `21986`：

```bash
nft add rule ip hy2jump prerouting udp dport 49000-50000 counter redirect to :21986
```
...
table ip hy2jump {
    chain prerouting {
        ...
        udp dport 49000-50000 counter redirect to :21986
    }
}
```

如果你的 HY2 真实端口不是 `21986`，只修改最后一条命令中的目标端口。
...
```bash
nft add rule ip hy2jump prerouting udp dport 49000-50000 counter redirect to :37873
```
...
如果服务器由 nftables 直接管理防火墙，可以先备份原配置：

```bash
cp -a /etc/nftables.conf "/etc/nftables.conf.bak.$(date +%Y%m%d-%H%M%S)" 2>/dev/null || true
```
...
> `nft list ruleset > /etc/nftables.conf` 会使用当前完整规则覆盖配置文件。如果服务器正在由 UFW、firewalld、Docker 或其他程序集中管理 nftables，请先备份并确认不会产生冲突。
...
客户端会在指定端口范围中随机选择端口，并按照一定时间间隔切换。

如果客户端只能填写单个数字端口，说明该客户端或当前配置方式可能不支持 HY2 多端口地址。
...
或者更换支持 Hysteria2 端口跳跃的客户端。

## 9. Cloudflare 必须使用 DNS Only
...
普通 Cloudflare 橙色云朵不能直接代理任意 HY2 UDP 端口。

因此建议单独创建一个 HY2 子域名，例如：
